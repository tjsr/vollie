...
export const EventFormPage = ({ currentUser, setFooters }: EventFormPateProps ): JSX.Element => {
  setFooters([<div><Link to="/events">Back to Events</Link></div>]);
  ...
  // Queries
  const eventQuery = useQuery({ queryKey: ['events', eventId], queryFn: () => {
  ...
  const organisationQuery = useQuery({ queryKey: ['organisations'], queryFn: () => {
    console.log('Fetching for All Orgs');
    return fetchOrganisations(currentUser);
  }});

  const seriesQuery = useQuery({ queryKey: ['series'], queryFn: () => {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      if (userOrgs === undefined || userOrgs.length <= 0) {
        console.log(`Insufficient oganisations for event details form - ${organisations?.length || 0} orgs and ${series?.length || 0} series.`);
      } else {
        console.log(`Loading event form schema with currentUser and ${userOrgs?.length || 0} orgs and ${userSeries?.length || 0} series.`);
  ...
  useEffect(() => {
    ...
    const retrievedUserOrgs = organisationQuery.data?.filter(
      (org: Organisation) => {
        const userOrgs:Organisation[] = currentUser?.organisations?.filter(
          (userOrg: Organisation) => userOrg.id === org.id
        ) || [];
  ...
  useEffect(() => {
    ...
    const retrievedUserSeries = seriesQuery.data?.filter(
      (series: Series) => {
        series.organiser === undefined || series.organiser === null || userOrgs?.map((org) => org.id).includes(series.organiser?.id)
      }) || [];
  ...
  if (eventQuery.failureReason) {
    return <>
      <h2>Edit Event</h2>
      <div className="error access">Failed loading event {eventId}: {eventQuery.failureReason.message}</div>
  ...
  return (<>
      <h2>{ eventIdInt !== undefined ? 'Edit' : 'Create' } Event</h2>
      { !eventFormSchema ? <div>Loading form schema...</div> : <div>
